...
The second was a style gap. The base wrote long suites with membership asserts; the metric rewarded short suites of exact-value asserts. Four thousand and twelve thousand teacher examples closed that gap for +0.059 and +0.062. A paragraph of system prompt closed it for +0.076. The 12,000 adapter under that same prompt scored 0.667, its own number under the plain prompt, and −0.014 against the prompt alone with an interval including zero: the two teach the same thing and do not stack. If the gap is style or format, prompt first, and fine-tune only when the prompt fails to reach it.
