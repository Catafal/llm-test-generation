...
ARMS = [  # label, run dir, hero?
    ...
    ("SFT · 12K KODCODE", "baselines-test-finetune-20260914T115239Z", True),
]
DEV = {
...
def chart_results(arms: list[dict]) -> Path:
    ...
    script = """
obsReveal('ch',s=>{
  const y0=i=>44+i*44,X0=118,PX=3.7,ORIGIN=40; // ticks start at 40 points: nothing scored below it
  ...
  el(s,'line',{x1:xOf(D[0][1]),y1:36,x2:xOf(D[0][1]),y2:y0(5)+22,stroke:FLOOR,'stroke-width':.7,'stroke-dasharray':'2 3',class:'fade',style:'animation-delay:.8s'});
  txt(s,{x:200,y:308,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle','letter-spacing':'.12em',class:'fade',style:'animation-delay:.9s'},
    ...
    return _page(
        ...
        "grounded score = mutants caught per function, 0 if the suite fails after the harness fills values · test split, 315 functions, Qwen3.5-4B · orange = the shipped adapter",
...
def chart_harness(arms: list[dict]) -> Path:
    ...
    script = """
obsReveal('ch',s=>{
  const y0=i=>50+i*44,X0=126,X1=372,mapX=v=>X0+(v-35)/50*(X1-X0);
  ...
  txt(s,{x:200,y:314,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle','letter-spacing':'.12em',class:'fade',style:'animation-delay:1s'},
    ...
    return _page(
        ...
        "share of suites that pass on the correct function, as written and after the harness rewrites literal expected values from execution · every arm gets the same harness",
...
def chart_discordant(arms: list[dict]) -> Path:
    ...
    script = """
obsReveal('ch',s=>{
  const x0=i=>56+i*70,base=250,step=3.6,HW=10;
  D.forEach(([name,was,now,hero],i)=>{
...
def chart_styles(arms: list[dict]) -> Path:
    ...
    pick = [arms[0], arms[3], arms[4], arms[5]]
    data = [
        [
            a["label"].replace("BASE ", "").replace(" KODCODE", ""),
