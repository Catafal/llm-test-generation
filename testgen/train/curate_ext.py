...
def stage_c(args) -> int:
    ...
    with (EXT_DIR / "train.jsonl").open("w") as tr, (EXT_DIR / "valid.jsonl").open("w") as va:
        for k in kept:
            line = json.dumps(to_chat(k["source"], k["suite"])) + "\n"
            (va if k["family"] in valid_fams else tr).write(line)
    (EXT_DIR / "pool.jsonl").write_text("".join(json.dumps(k) + "\n" for k in kept))
    ...
    (EXT_DIR / "yield.json").write_text(json.dumps(summary, indent=1))
    (EXT_DIR / "NOTICE.md").write_text(
    ...
    (EXT_DIR / "DECONTAMINATION.md").write_text(
